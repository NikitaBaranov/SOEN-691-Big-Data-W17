...
def assign_clusters (clusteroids_to_assign, dataframe):
    ...
    logger.info("Assign clusters - begin")
    for point in dataframe.index.values: # Iterete through data to clusterise
        ...
        for clusteroid in clusteroids_to_assign: # iterate throught clusters
            distance = haming_distance(dataframe.iloc[point][columns_to_count_distance],
                              dataframe.iloc[clusteroid][columns_to_count_distance])
        ...
        logger.info("Point {} assignbed to {} with distance {}".format(point, min_clusteroid, min_distance))
    print ""
    print "Assign clusters - end"
    logger.info("Assign clusters - end")
...
def recalculate_clusters (clusteroids, dataframe):
    ...
    logger.info("Clusters {}".format(clusteroids))
    ...
    for clusteroid in old_clusteroids: # Iterete through to clusteroids
        ...
        logger.info("Cluster {}".format(clusteroid))
        ...
        for element_i in dataframe[dataframe[cluster_field] == clusteroid].index.values: # iterate throught cluster
            ...
            for element_j in dataframe[dataframe[cluster_field] == clusteroid].index.values: # iterate throught cluster
                distance = haming_distance(dataframe.iloc[element_i][columns_to_count_distance],
                                           dataframe.iloc[element_j][columns_to_count_distance])
            ...
            if sum_distance < min_distance:
                ...
                logger.info("----New clusteroid {} min summ distance {}".format(element_i,sum_distance))
        clusteroids.append(min_clusteroid)
        clusters_density.append(min_distance)
    print "Old Clusters {}".format(old_clusteroids)
    logger.info("Old Clusters {}".format(old_clusteroids))
    print "Clusters {}".format(clusteroids)
    logger.info("Clusters {}".format(clusteroids))
    print "---------"
...
log_file_name = "{}-{}-{}_{}:{}_{}_{}_{}.log".format(
    i.year,
    i.month,
    i.day,
    i.hour,
...
logger.info("Finde Clusteroids")
print "Read from file temp_uc_kep_obs.txt_5000-500.csv"
logger.info("Read from file temp_uc_kep_obs.txt_5000-500.csv")

df_to_cluster = pd.read_csv("temp_uc_kep_obs.txt_5000-500.csv", encoding= "utf-8", infer_datetime_format = True, low_memory=False)

#continue Clustering

print ""
print "Clssterisation - k-maens - beginning"
logger.info("Clssterisation - k-maens - beginning")
print "Assign random clusters"
logger.info("Assign random clusters")
...
while (clusteroids_array != new_clusteroids_array):
    ...
    logger.info("Run N {}".format(step))
    logger.info("Run N {} assign_clusters call ".format(step))
    assign_clusters(clusteroids_array, df_to_cluster)
    logger.info("Run N {} assign_clusters return ".format(step))
    logger.info("Run N {} recalculate_clusters call ".format(step))
    new_clusteroids_with_density = recalculate_clusters(clusteroids_array, df_to_cluster)
    logger.info("Run N {} recalculate_clusters return ".format(step))
    ...
    logger.info("Run {} Intermidiate clusteroids {}".format(step,new_clusteroids_array))
    logger.info("Run {} Intermidiate cluster density {}".format(step,new_clusters_density))
    step = step + 1
print "Clasterisation - end."
logger.info("Clasterisation - end.")
print "Final clusteroids {}".format(new_clusteroids_array)
logger.info("Final clusteroids {}".format(new_clusteroids_array))
print "Final cluster density {}".format(new_clusters_density)
logger.info("Final cluster density {}".format(new_clusters_density))
...
logger.info("Result in {}".format(file_name_to_save))
print ""
