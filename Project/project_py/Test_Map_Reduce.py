...
def assign_map (row):
    ...
    #min_clusteroid = -1
    result=[]
    for clusteroid in broadcast_clusteroids.value: # iterate throught clusters
        distance = haming_distance(broadcast_data[row],broadcast_data[clusteroid])
    ...
    print "{}-{}".format(row,min_clusteroid),
    return [row,[min_clusteroid,min_distance]]

# Assign to clusters
def assign_clusters (clusteroids_to_assign, dataframe):
    print "Assign clusters - begin"
    broadcast_data = sc.broadcast(spark.createDataFrame(dataframe))
    broadcast_clusteroids = sc.broadcast( )
    points_after_map = dataframe.index.values.rdd.map(lambda x: (assign_map(x)))
    #points_after_reduce = points_after_map.reduceByKey(lambda x: (assign_map(x)))        
    print ""
...
print "Read from file temp_uc_kep_obs.txt_1000.csv"

df_to_cluster = pd.read_csv("Data/fakeData_rus.csv", encoding= "utf-8", infer_datetime_format = True, low_memory=False)
